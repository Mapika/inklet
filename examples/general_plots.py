...
def make_document(*,legacy=False):
    tables=load_data()
    style=i.preset('scientific.general').customize(palette=('#24698c','#288675','#b86443'))
    ...
    def axes(plot,**kwargs):
        if not legacy:
            plot.grid(x=False,count=4,stroke='#e1e7e4',stroke_width=style.theme.hairline)
        return plot.axes(**kwargs,**axis_options)
    def legend(plot,**placement):
        if legacy:return plot.legend(side='bottom',**legend_options)
        return plot.legend(**placement)
    learning=tables['learning']
    p=i.plot_spec(height=34,x=(0,60),y=(0,1.2))
    ...
    axes(p,x='Epoch',y='Training loss');legend(p,corner='ne')
    doc.add('learning',p,row=0,column=0)
    benchmark=tables['benchmark']
    p=i.plot_spec(height=34,x=(10,45),y=(.75,.96))
    ...
    for index,name in enumerate(benchmark.columns['model']):
        ...
        p.scatter(points,name=name,color=colors[name],size=2.5,stroke_width=style.theme.hairline)
    ...
    p=i.plot_spec(height=34,x=(0,6),y=(-.1,1.15))
    p.scatter(sensor.points('time','measured'),name='Samples',size=.8,color='#94b7c4',stroke_width=style.theme.hairline)
    p.line(sensor.points('time','model'),name='Model',stroke='#24698c',stroke_width=.4)
    axes(p,x='Time / s',y='Normalized response');legend(p,corner='se')
    ...
    p=i.plot_spec(height=34,x=revenue.columns['quarter'],y=products[::-1])
