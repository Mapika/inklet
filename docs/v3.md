---
archived: true
current: three-images.md
---

# Rendering in v3

**Inklet 3.0** adds a complete scene-rendering workflow alongside the existing
plotting and document APIs. Install with `python -m pip install 'inklet[render]'`.
See [migration from 2.6](migration.md#from-26-to-30) and the
...
Dev4 adds [camera-aware vector paths](scene-paths.md). Project world points from
a saved snapshot, check visibility against the depth pass, and draw paths whose
occluded sections are omitted or dashed. Both camera types retain vector paths
in SVG and PDF; changing an overlay does not rerender the scene.

![Two Blender camera views with projected labels, a plot and vector paint](../gallery/v3-rendering.png)

Dev5 adds [scene annotations and measurements](scene-annotations.md): projected
labels, depth-tested arrows, true 3D lengths and world-plane angle arcs.
Text and annotation strokes remain in page units and vector exports.

Dev6 adds [packaged scene templates](scene-templates.md) for laboratory apparatus,
product presentation and architecture, plus a [complex annotated laboratory](complex-scene.md).
Templates include validated parameters, named cameras, landmarks and generation provenance.
...
The unreleased [hatch engine review](hatching.md) describes the next improvements
to fill/border opacity, shared vector resources and transformed hatch fidelity.
