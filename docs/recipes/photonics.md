...
The chip, waveguides and fibres are an original conceptual illustration. The companion curve multiplies two analytic Lorentzian dips; it is not an optical simulation of the geometry.
...
python tools/showcase_gallery.py --only photonics --quality final
```

Open `out/showcase/photonics/figure.html` to review the result. The same folder contains `figure.svg`, `figure.pdf` and `figure.png`.
