...
Preview schemas are versioned independently. Bundles contain data and choices,
...
class FigureProject:
    ...
    def open(cls, directory, factory, *, verify_export=True):
        ...
        if verify_export and hashlib.sha256(result.editor.figure.to_svg().encode()).hexdigest() != value['svg_sha256']:
            raise ValueError('reconstructed figure differs; check recipe, Inklet version and fonts')
        return result
