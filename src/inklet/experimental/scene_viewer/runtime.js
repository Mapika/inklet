...
class CompiledSceneViewer{
 constructor(host,scene){
  ...
  this.host=host;this.scene=scene;this.backend=scene.backend;this.layers=[];this.uploadBytes=0;this.drawCalls=0;this.disposed=false;this.frame=0;
 ...
 canvasLayer(layer,reason){
  ...
  layer.context=canvas.getContext('2d');if(!layer.context)throw Error('Canvas 2D unavailable');layer.reason=reason;layer.actual='canvas';
 }
 setBackend(name){
 ...
 render(){
  ...
  for(const layer of this.layers){
   if(layer.actual==='svg')continue;
   const m=layer.foreign.getScreenCTM(),b=layer.batch.box;if(!m)continue;
   ...
   const width=Math.max(1,Math.round(b[2]*scale*reduction)),height=Math.max(1,Math.round(b[3]*scale*reduction));
 ...
 report(){return {backend:this.backend,layers:this.layers.map(l=>({backend:l.actual,count:l.batch.count,reason:l.reason,renderer:l.gpu?.renderer||null,reducedResolution:!!l.reduced})),native:this.scene.native,uploadBytes:this.uploadBytes,drawCalls:this.drawCalls};}
 setViewport(v){
