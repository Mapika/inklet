...
3. The first free candidate on the first ring wins. Otherwise every
   ...
   lowest total wins; a label whose winner still overlaps or crosses
   something is listed as unresolved in the node's `point_labels` note.

Points are labelled most crowded first, so the points with the fewest free
positions choose before their neighbours take them. Ties keep input order.
...
#: Scores: an overlap in square millimetres counts this much, a segment
#: crossing a fixed amount, and each millimetre of distance from the point
#: this much -- so a free position 7 mm out loses to a near one that grazes
#: 0.2 mm^2 of a background mark.
...
def label_points(panel, points: Sequence[Sequence], labels: Sequence[str], *,
    ...
    for index in order:
        ...
        best = None
        for distance, angle, first in _candidates(gap + radii[index], ring, far):
            box = _box_at(anchors[index], angle, distance, width, height)
            if not _within(box, area):
                continue
            score = 0.0
            # Everything except the point's own marker is kept `spacing`
            # away; the own marker only must not be covered, since the first
            # ring already sits `clear` from it.
            ...
            for other in index_of.near(spaced):
                if id(other) in own_marker[index]:
                    continue
                # Labelled points' markers are in `targets` below, kept the
                # full gap away; other marks only a fraction of it, so a
                # label can still sit in a gap in a dense cloud.
                score += _OVERLAP_WEIGHT * _overlap(padded, other)
            for number, other in enumerate(targets):
                score += _OVERLAP_WEIGHT * _overlap(
                    box if number == index else spaced, other)
            for other in placed.values():
                score += _OVERLAP_WEIGHT * _overlap(spaced, other)
            score += _CROSSING_WEIGHT * sum(
                1 for a, b in segments if _segment_hits(a, b, box))
            score += _CROSSING_WEIGHT * sum(
                1 for a, b in leaders.values() if _segment_hits(a, b, box))
            line = None
            if not first and leader:
                line = _leader(anchors[index], box, radii[index] * _STANDOFF)
                if line is not None:
                    score += _CROSSING_WEIGHT * sum(
                        1 for other in placed.values()
                        if _segment_hits(line[0], line[1], other))
                    score += _CROSSING_WEIGHT * sum(
                        1 for a, b in leaders.values()
                        if _segments_cross(line[0], line[1], a, b))
                    score += _CROSSING_WEIGHT * sum(
                        1 for number, other in enumerate(targets)
                        if number != index
                        and _segment_hits(line[0], line[1], other))
            if score == 0.0 and first:
                best = (0.0, box, line, 0.0)
                break
            total = score + _DISTANCE_WEIGHT * distance
            if best is None or total < best[0]:
                best = (total, box, line, score)
        if best is None:
            raise DiagramError(
...
def _candidates(start: float, ring: float, far: float):
    """`(distance, page angle, first ring?)` in search order."""
    for angle in _FIRST:
        yield start, angle, True
    distance = start + ring
    while distance <= far + 1e-9:
        for step in range(16):
            yield distance, _FIRST[step % 8] if step < 8 else _FIRST[step % 8] + 22.5, False
        distance += ring
