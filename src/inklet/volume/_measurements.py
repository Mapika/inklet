...
from ._sections import SampledSection
...
def measure_labels(labels, channels, *, label_ids=None, region=None, coverage='intersection'):
    ...
    if isinstance(labels,Volume):
        def geometry(v):
            return (v.data.shape,v.spacing_zyx,v.origin_xyz,v.unit)
        if labels.data.dtype.kind not in 'bui':
        ...
        element_measure=math.prod(labels.spacing_zyx)
        unit=labels.unit
        dimension=3
        ...
        element_measure=math.prod(labels.plane.spacing_yx)
        unit=labels.plane.unit
        dimension=2
        if region is not None:
            _region(region,unit)
            selected &= region.mask(labels.plane)
    ...
    for name,channel in channels.items():
        ...
        np.minimum.at(low,group,values)
        np.maximum.at(high,group,values)
        with np.errstate(over='ignore',invalid='ignore'):
