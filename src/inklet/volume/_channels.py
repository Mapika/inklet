...
from ._sections import SampledSection, _snapshot
from ._slabs import SlabProjection
...
class Channel:
    ...
    def __post_init__(self):
        ...
        if weight>1:
            raise ValueError('Channel weight must be at most 1')
        if not isinstance(self.color,str):
            raise ValueError('Channel color must be a CSS color string')
    ...
    def report(self):
        np=_numpy()
        values=self.sampled.data[self.sampled.valid]
