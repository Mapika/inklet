...
from ._volume import _numpy, _positive
...
class LabelContour:
    ...
    def world_segments(self, *, include_coverage=False):
        ...
        if type(include_coverage) is not bool:
            raise ValueError('include_coverage must be boolean')
    ...
    def diagram(self, *, width, stroke, stroke_width=.25, coverage='omit'):
        ...
        for name,edges in [('observed',self.observed_yx),('coverage',self.coverage_yx)]:
            if not edges or (name=='coverage' and coverage=='omit'):
                continue
