...
from ._volume import Volume, _numpy
...
def read_tiff(path, *, spacing_zyx, unit, source_id, channel_names=None,
    ...
    if type(series) is not int or series<0:
        raise ValueError('series must be a nonnegative integer')
    ...
    if isinstance(channel_names,str):
        raise ValueError('channel_names must be a sequence of names')
    path=Path(path)
        ...
        with tifffile.TiffFile(stream, _multifile=False) as source:
            ...
            if 'T' in declared:
                if time_index is None:
                    raise ValueError('A TIFF with a T axis requires an explicit time_index')
                if time_index>=shape[declared.index('T')]:
    ...
    used_axes=declared
    if 'T' in used_axes:
        array=np.take(array,time_index,axis=used_axes.index('T'))
        used_axes=used_axes.replace('T','')
    for axis in 'CZ':
        if axis not in used_axes:
            array=np.expand_dims(array,0)
            used_axes=axis+used_axes
    ...
    if not isinstance(source_id,str) or not source_id.strip():
        raise ValueError('source_id must identify the TIFF source')
