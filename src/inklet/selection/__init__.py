...
A `KeyedTable` snapshots JSON scalar columns under an explicit string row key;
a `SelectionState` records selected and visible row IDs tied to the table's
content digest and serializes as `inklet.selection/0.1` JSON. Identity is never
inferred from row position, and changed contents are rebound only through an
explicit `SelectionState.rebase`.

These are the row-identity layer for linked views and figure projects; plots
take their columns from `inklet.Dataset` instead. Available in 4.3; the same
objects remain importable from `inklet.experimental.selection`.
"""
...
__all__ = ['KeyedTable', 'SelectionState', 'RebasedSelection']
...
class KeyedTable:
    ...
    def from_pandas(cls, name, frame, *, key='id', columns=None, time_columns=None):
        ...
        Imports pandas only when called.
        """
